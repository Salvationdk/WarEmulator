...
namespace FrameWork.NetWork.Crypt.Crypto
{
    public class ByteSwap
    {
        ...
        public static double Swap(double input)
        {
            ...
            return (double)BitConverter.ToSingle(new byte[] { bytes[7], bytes[6], bytes[5], bytes[4], bytes[3], bytes[2], bytes[1], bytes[0] }, 0);
        }
        ...
        {
            byte[] bytes = BitConverter.GetBytes(input);
            byte[] buffer2 = new byte[BitConverter.GetBytes(input).Count<byte>()];
            for (int i = 0; i < BitConverter.GetBytes(input).Count<byte>(); i++)
            {
                buffer2[i] = bytes[(BitConverter.GetBytes(input).Count<byte>() - 1) - i];
            }
