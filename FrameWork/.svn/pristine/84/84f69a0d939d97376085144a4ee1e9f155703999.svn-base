...
namespace FrameWork
{
    public class XMLObjectDatabase : ObjectDatabase
    {
        ...
        protected override bool AddObjectImpl(DataObject dataObject)
        {
            ...
            {
                Log.Error("XMLObjectDatabase", "AddNewObject " + e);
        ...
        protected override DataObject[] SelectObjectsImpl(Type objectType, string statement, IsolationLevel isolation)
        {

            string tableName = GetTableOrViewName(objectType);

            Log.Debug("XMLObjectDatabase","1. select objects " + tableName + " " + statement);
            ...
            if (ds != null)
            {
                ...
                Log.Debug("XMLObjectDatabase", "2. " + tableName + " " + statement);

                DataRow[] rows = table.Select(statement);

                Log.Debug("XMLObjectDatabase", "3. " + tableName + " " + statement);
                ...
                Log.Debug("XMLObjectDatabase", "4. select objects " + tableName + " " + statement);
        ...
        /// <param name="whereClause">the where clause to filter object selection on</param>
        /// <returns>an array of <see cref="DataObject" /> instances representing the selected objects that matched the given criteria</returns>
        protected override IList<TObject> SelectObjectsImpl<TObject>(string statement, IsolationLevel isolation)
        {
            string tableName = GetTableOrViewName(typeof(TObject));
        ...
        protected override IList<TObject> SelectAllObjectsImpl<TObject>(IsolationLevel isolation)
        {
            ...
            Log.Debug("XMLObjectDatabase", "1. select all " + tableName);
            ...
            if (ds != null)
            {
                ...
                Log.Debug("XMLObjectDatabase", "2. select all " + tableName);
        ...
        {
            Log.Debug("XMLObjectDatabase", "Raw query execution not supported in XML mode.");
        ...
        public void SaveFile(string TableName)
        {
            DataSet Data = GetDataSet(TableName);
            if (Data != null)
            {
                Connection.SaveDataSet(TableName, Data);
            }
        }
    }
}
