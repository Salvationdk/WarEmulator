...
namespace FrameWork.NetWork.Crypt.Crypto
{
    public class Sha256Digest : GeneralDigest
    {
        ...
        {
            base.Finish();
            ByteSwap.UInt32_To_BE(this.H1, output, outOff);
            ByteSwap.UInt32_To_BE(this.H2, output, outOff + 4);
            ByteSwap.UInt32_To_BE(this.H3, output, outOff + 8);
            ByteSwap.UInt32_To_BE(this.H4, output, outOff + 12);
            ByteSwap.UInt32_To_BE(this.H5, output, outOff + 0x10);
            ByteSwap.UInt32_To_BE(this.H6, output, outOff + 20);
            ByteSwap.UInt32_To_BE(this.H7, output, outOff + 0x18);
            ByteSwap.UInt32_To_BE(this.H8, output, outOff + 0x1c);
            this.Reset();
            return 0x20;
        }
        ...
        {
            this.X[this.xOff] = ByteSwap.BE_To_UInt32(input, inOff);
            if (++this.xOff == 0x10)
            {
                this.ProcessBlock();
            }
